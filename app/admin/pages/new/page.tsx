...
export default function NewPagePage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!res.ok) {
        setToast({ type: "error", message: data.error ?? "Failed to save page." });
        return;
      }
      setToast({ type: "success", message: "Page created successfully!" });
      setTimeout(() => router.push(`/admin/pages/${data.id}`), 1000);
    } catch {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-black text-white">New Page</h1>
          <p className="text-sm mt-0.5" style={{ color: "#9ca3af" }}>Create a new static page</p>
        </div>
      </div>
          ...
          <div className="rounded-xl p-5 space-y-4" style={{ background: "#141422", border: "1px solid #1e1e2e" }}>
            ...
            <div>
              ...
              <p className="text-xs mt-1" style={{ color: "#4b5563" }}>
                This page will be accessible at <code className="text-purple-400">/{form.slug || "page-slug"}</code>
              </p>
            </div>
            <div>
